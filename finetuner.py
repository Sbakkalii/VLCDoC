...
early_stop = EarlyStopping(monitor='SupCon_loss', patience=10, mode='min', verbose=1, restore_best_weights=True),
...
lr_decay = ReduceLROnPlateau(factor=np.sqrt(0.5), monitor="SupCon_loss", mode="min", cooldown=0, patience=5,
...
data_valid = create_rvlcdip_dataset(path_train, "Test_Data")
data_test = create_rvlcdip_dataset(path_train, "Test_Data")
_, data_valid = train_test_split(data_valid, test_size=0.2, random_state=len(data_valid))
_, data_test = train_test_split(data_test, test_size=0.2, random_state=len(data_test))
...
class ContrastiveModel(tf.keras.Model):
    def __init__(self, encoder, classifier):
        ...
        self.intra_image_projection_head = tf.keras.Sequential(
            [
                ...
                layers.Dense(width, activation="tanh"),
                layers.Dense(width),
            ],
    ...
    def Image_Cross_SupCon_loss(self, labels, image_feature_vectors, text_feature_vectors):
        ...
        text_logits = tf.divide(
            tf.matmul(
                image_feature_vectors_normalized, tf.transpose(text_feature_vectors_normalized)
            ),
            self.temperature,
        )
    ...
    def Text_Cross_SupCon_loss(self, labels, text_feature_vectors, image_feature_vectors):
        ...
        text_cross_logits = tf.divide(
            tf.matmul(
                text_feature_vectors, tf.transpose(image_feature_vectors_normalized)
            ),
            self.temperature,
        )
...
def main():
    train_dataset = tf.data.Dataset.from_generator(train_batchgen_gen, output_types=(tf.uint8, tf.int64, tf.int64, tf.int64))
    train_dataset = train_dataset.batch(batch_size)
    train_dataset = train_dataset.prefetch(AUTO)
    train_dataset = train_dataset.with_options(options)
    
    valid_dataset = tf.data.Dataset.from_generator(valid_batchgen_gen, output_types=(tf.uint8, tf.int64, tf.int64, tf.int64))
    valid_dataset = valid_dataset.batch(batch_size)
    valid_dataset = valid_dataset.prefetch(AUTO)
    valid_dataset = valid_dataset.with_options(options)
    
    test_dataset = tf.data.Dataset.from_generator(test_batchgen_gen, output_types=(tf.uint8, tf.int64, tf.int64, tf.int64))
